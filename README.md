...
VolDy-VAE: A variational autoencoder-based approach for probabilistic time series forecasting with location-scale Gaussian observation modeling.
...
| File                      | Description                                             |
| ------------------------- | ------------------------------------------------------- |
| `VolDy_VAE_nn.py`         | Core VAE architecture implementation                    |
| `VolDy_VAE_Forecaster.py` | ProbTS forecaster wrapper with training/inference logic |
| `VolDy_VAE.yaml`          | Configuration file for training                         |
...
Key hyperparameters in `VolDy_VAE.yaml`:
